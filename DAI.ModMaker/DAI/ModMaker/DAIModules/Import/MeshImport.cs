	...
	public class MeshImport : DAIBaseImporter
	{
		private void ComputeTangentBasis(ref MeshBuffer MeshBuffer)
		{
			...
			for (int k = 0; k < MeshBuffer.VertexBuffer.Count; k++)
			{
				...
				MeshBuffer.VertexBuffer[k].Tangents = new DAI.AssetLibrary.Assets.Bases.Vector3
				{
					X = vector6.X,
					Y = vector6.Y,
					Z = vector6.Z
				};
				MeshBuffer.VertexBuffer[k].Bitangents = new DAI.AssetLibrary.Assets.Bases.Vector3
				{
					X = vector7.X,
					Y = vector7.Y,
					Z = vector7.Z
		...
		private ChunkRef AddNewChunk(ChunkRef oldChunk, byte[] chunkPayload, int meshNameHash, ResRef res)
		{
			...
			byte[] modifiedData = DAI.AssetLibrary.Utilities.Utilities.CompressData(chunkPayload);
